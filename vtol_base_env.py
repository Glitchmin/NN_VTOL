...
class VTOLBaseEnv(gymnasium.Env):
    ...
    def __init__(
        ...
        """
        if 480 % agent_hz != 0:
            lowest = int(480 / (int(480 / agent_hz) + 1))
            highest = int(480 / int(480 / agent_hz))
            raise ValueError(
        ...
        self.env_step_ratio = int(480 / agent_hz)
        if angle_representation == "euler":
    ...
    def begin_reset(
        ...
        drone_options["model_dir"] = "Tricopter_VTOL"

        # init env
        self.env = Aviary(
            start_pos=self.start_pos,
            start_orn=self.start_orn,
            drone_type="tricopter_VTOL",
    ...
    def end_reset(
        ...
        for _ in range(200):
            self.env.set_setpoint(0, np.array([0, 0, 0, 0, 0, 0, 0]))
            self.env.step()
        self.compute_state()
